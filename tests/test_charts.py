...
class TestCharts(unittest.TestCase):
    ...
    def init(self, start=True):
        ...
        self.cnx = sqlite3.connect(
            os.path.join(_charts.DATABASE_PATH, _charts.DATABASE_NAME)
        )
    ...
    def __fill_data_table(
        self,
        table_name,
        values,
        event_name="test.test.test",
        fields_name=["field1", "field2", "field3", "field4", "field5"],
    ):
        """
    ...
    def test_save_data_1(self):
        ...
        self.assertEqual(
            row[0][3], values[0]["field"], "Field name is not properly saved"
        )
    ...
    def test_save_data_2(self):
        ...
        self.assertEqual(
            row[0][4], values[1]["field"], "Field name is not properly saved"
        )
        self.assertEqual(row[0][5], None, "Field name is not properly saved")
        self.assertEqual(row[0][6], None, "Field name is not properly saved")
        count = self.__get_table_count("data1")
    ...
    def test_save_data_3(self):
        ...
        self.assertEqual(
            row[0][5], values[2]["field"], "Field name is not properly saved"
        )
        self.assertEqual(row[0][6], None, "Field name is not properly saved")
        count = self.__get_table_count("data1")
        self.assertEqual(count, 0, "Data1 table should have no record")
    ...
    def test_save_data_4(self):
        ...
        self.assertEqual(
            row[0][6], values[3]["field"], "Field name is not properly saved"
        )
        count = self.__get_table_count("data1")
        self.assertEqual(count, 0, "Data1 table should have no record")
        count = self.__get_table_count("data2")
    ...
    def test_save_data_bool(self):
        ...
        uuid = "123-456-789-2"
        values = [
            {"field": "test1", "value": True},
            {"field": "test2", "value": True},
            {"field": "test3", "value": True},
        ]
        self.module._save_data(uuid, event, values)
        ...
        uuid = "123-456-789-4"
        values = [
            {"field": "test1", "value": False},
            {"field": "test2", "value": False},
            {"field": "test3", "value": False},
            {"field": "test4", "value": False},
        ]
        self.module._save_data(uuid, event, values)
    ...
    def test_save_data_invalid_parameters(self):
        ...
        self.assertEqual(
            cm.exception.message,
            'Parameter "device_uuid" is missing',
        ...
        self.assertEqual(
            cm.exception.message, "No value to save", "Should failed if values if empty"
        )
        with self.assertRaises(InvalidParameter) as cm:
            self.module._save_data(uuid, event, [{}, {}, {}, {}, {}])
        self.assertEqual(
            ...
            % (event, [{}, {}, {}, {}, {}]),
            "Should failed if too many values are passed",
        )
    ...
    def test_get_data_1(self):
        ...
        self.assertEqual(
            len(data["names"]),
            len(values) + 1,
            "There should have %d names" % (len(values) + 1),
        )
        self.assertTrue(
            "timestamp" in data["names"], "ts column should be returned in names"
        )
        self.assertTrue(
            "test1" in data["names"], "test1 column should be returned in names"
        )
        for name in [value["field"] for value in values]:
            self.assertTrue(
                name in data["names"], 'Name "%s" should exists in names' % name
            )
        for value in [(value["field"], value["value"]) for value in values]:
            self.assertTrue(
                value[0] in data["data"][0],
                'Name "%s" should exists in data' % value[0],
            )
            self.assertEqual(
                value[1],
                data["data"][0][value[0]],
                'Invalid value saved for field "%s"' % value[0],
            )
    ...
    def test_get_data_with_options(self):
        ...
        self.assertTrue(
            "test2" in data["names"], "test2 column should only be returned"
        )
        self.assertTrue("test2" in data["data"][0], "test2 column should be returned")
        self.assertTrue(
            "test1" not in data["data"][0], "test1 column should not be returned"
        )
        ...
        self.assertTrue(
            "test1" in data["data"], "test1 colum should be returned in data dict"
        )
        self.assertTrue(
            "test2" in data["data"], "test1 colum should be returned in data dict"
        )
        self.assertTrue(
            "values" in data["data"]["test1"], 'Data should contain "values" key'
        )
        self.assertTrue(
            "name" in data["data"]["test1"], 'Data should contain "name" key'
        )
        self.assertTrue(
            isinstance(data["data"]["test1"]["values"], list),
            "Data should be returned as list",
        )
    ...
    def test_get_data_invalid_parameters(self):
        ...
        self.assertEqual(
            cm.exception.message,
            'Parameter "timestamp_start" is missing',
            "Timestamp_start should not be None",
        )
        with self.assertRaises(MissingParameter) as cm:
            self.module.get_data(uuid, start, None)
        self.assertEqual(
            cm.exception.message,
            'Parameter "timestamp_end" is missing',
            "Timestamp_end should not be None",
        )
        with self.assertRaises(InvalidParameter) as cm:
            self.module.get_data(uuid, -1, end)
        self.assertEqual(
            cm.exception.message,
            "Timestamp_start value must be positive",
            "Timestamp_start should be >0",
        )
        with self.assertRaises(InvalidParameter) as cm:
            self.module.get_data(uuid, start, -1)
        self.assertEqual(
            cm.exception.message,
            "Timestamp_end value must be positive",
            "Timestamp_end should be >0",
        )
    ...
    def test_purge_data_missing_parameters(self):
        ...
        self.assertEqual(
            cm.exception.message,
            'Parameter "timestamp_until" is missing',
            "Timestamp_until should not be None",
        )
        with self.assertRaises(InvalidParameter) as cm:
            self.module.purge_data(uuid, -1)
        self.assertEqual(
            cm.exception.message,
            "Timestamp_until value must be positive",
            "Timestamp_until should be >0",
        )
    ...
    def test_event_received(self):
        ...
        self.assertEqual(
            ...
            1,
            "Get_event_instance should be called",
        )
        count = self.__get_table_count("data1", uuid)
    ...
    def test_event_received_single_false_value(self):
        ...
        self.assertEqual(
            rows[1][3], 0, "0 value should be inserted instead of real value"
        )
